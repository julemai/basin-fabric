...
#
# ------------------------
#        NetCDF inputs :: RDRS-v2.1
#        Shapefile     :: JSON
# ------------------------
# run derive_grid_weights.py -i ../../data_in/data_obs/01010000/01010000_RDRS_v2.1_P_TT_1.5m_1980.nc -d "rlon,rlat" -v "lon,lat" -r ../../data_in/shapefiles/01010000/01010000_lp.json -b 01010000 -o ../../data_in/data_obs/01010000/01010000_RDRS_v2.1_GridWeights.txt -a  -c 'id'
...
parser.add_argument('-r', '--routinginfo', action='store',
                    ...
                    help='Shapefile that contains all information for the catchment of interest (and maybe some more catchments; can be shp or json).')
...
# -------------------------------
print(' ')
print('   (2) Reading shapefile data (shp or json) ...')
...
for ikk,kk in enumerate(keys):
    ...
    if abs(error) > area_error_threshold and area_basin > 500000.:
        ...
        for idata in data_to_write:
            ...
            #                                                             [int(ibasin[key_colname]),ilat,ilon,cell_ID,area_intersect/area_basin]
