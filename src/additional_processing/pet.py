...
def get_priestley_taylor_pet(t_min: np.ndarray, t_max: np.ndarray, s_rad: np.ndarray, lat: float, elev: float,
    ...
    # --- SAFETY CHECKS for polar night or numerical issues ---
    # In polar night, sha, et_rad, and cs_rad = 0 --> net_rad <= 0 --> set PET = 0
    ...
    denom = slope_svp + gamma
    denom = np.where(denom == 0, np.nan, denom)  # JMJMJM: avoid divide-by-zero
    
    pet = (alpha / _lambda) * (slope_svp * net_rad) / denom

    # convert energy to evap
    pet = pet * 0.408

    # clean output
    pet = np.where(np.isfinite(pet), pet, 0.0)   # JMJMJM 
    pet = np.where(pet < 0, 0.0, pet)            # JMJMJM
