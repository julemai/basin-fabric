...
def copyVariable(ncin, var, data=True, dims=False, vname=None, fail=True, **kwargs):
    """
    Arguments
    ---------
    ...
    vvname = invardef.pop("name")

    if not (vname is None):
        vvname = vname

    invar = ncin.createVariable(vvname, invardef.pop("dtype"), fail=fail, **invardef)
...
def getVariables(ncin):
    ...
    for v in getattr(ncin, "variables").values():
        out[v.name] = NcVariable(
            ncin, v.name, v.dtype, list(getDimensions(ncin).values()), id=v._varid
        )
    return out
...
def createVariable(ncin, varname, datatype, dimensions=(), fail=True, **kwargs):
    try:
        var = NcVariable(ncin, varname, datatype, list(getDimensions(ncin).values()), **kwargs)
